...
# ----------------------------------------------------------------------------------
# ABA 2: MARCAÇÃO INDIVIDUAL DE VOLUMOSOS (NOVA ABA)
# ----------------------------------------------------------------------------------
with tab2:
    st.header("2. Marcar Pacotes Volumosos por Gaiola (*)")
    st.caption("Use esta aba para marcar os pacotes volumosos (eles receberão um `*` no ID). **As marcações afetam a aba 'Pré-Roteirização'.**")
    ...
    if dfs_prontos_para_marcar:
        ...
        # Cria as sub-abas para cada arquivo carregado
        tab_titles = [f"Gaiola: {item['gaiola']} ({item['file_name']})" for item in dfs_prontos_para_marcar]
